...
YAD_2_SEARCH_URL = 'https://gw.yad2.co.il/feed-search-legacy/realestate/rent'
...
def _get_all_unit_data(units_metadata):
    ...
    for [id, _] in units_metadata:
        ...
        units.append(_get_unit_data(url))
...
def _extract_unit_data(unit):
    ...
    #print(unit['additional_info_items_v2']['air_conditioner'])
    #print(unit['additional_info_items_v2'])
    #print(unit['info_bar_items'][0]['value'])
    #print(unit['info_bar_items'])
    #print(unit['info_bar_items'])
    #print(unit['info_bar_items'][0]['titleWithoutLabel'])
    #print(items)
    #print(unit)
    #for u in unit:
        #print(u)
    #print(unit['important_info_items_v2'])
    #print(unit['price'])
    return {
    'city': unit['city_text'],
    ...
    'rooms': unit['info_bar_items'][0]['titleWithoutLabel'],
    #'rooms': unit['analytics_items']['rooms'],
    #'rooms': unit['info_bar_items'][0]['value'],
    'balconies': unit['balconies'],
    'air_condition': 1 if items['air_conditioner'] else 0,
    
    'parking': 1 if unit['parking'].isdigit() else 0,

    'protected_room': 1 if items['shelter'] else 0,
    'elevator': 1 if items['elevator'] else 0,
    'renovated': 1 if items['renovated'] else 0,
    'furniture': 1 if items['furniture'] else 0,
    'accessibility': 1 if items['accessibility'] else 0,
    'bars': 1 if items['bars'] else 0,
    #'accessibility': unit['analytics_items']['handicapped'],
    #'bars' : 1 if unit['additional_info_items_v2'][2]['value'] else 0,
    #'bars': unit['on_pillars'],
    #'storage': unit['analytics_items']['storeroom'],
    'storage': 1 if items['warhouse'] else 0,
